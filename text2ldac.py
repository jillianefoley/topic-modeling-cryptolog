...
# From https://github.com/JoKnopp/text2ldac
# Edited to ignore UTF-8 encoding issues
...
def load_stopwords(stopword_filename):
    ...
    with codecs.open(stopword_filename, 'r',  encoding='utf-8', errors='ignore') as sf:
        for line in sf:
            if len(line.split()) != 1:
...
def write_document_map_file(fnames, dmap_fname):
    ...
    """
    with codecs.open(dmap_fname,'w', encoding='utf-8', errors='ignore') as d_file:
        for title in fnames:
            d_file.write(title + '\n')
...
def generate_dat_lines_and_word_ids(fnames, config):
    ...
    for docname in fnames:
        ...
        try:
            with codecs.open(docname, 'r',  encoding='utf-8', errors='ignore') as doc:
                for line in doc:
                    for word in line.split():
                        ...
                        # Remove anything with crazy punctuation or numbers
                        if not re.compile('^[a-zA-Z\-\'\&]+$').match(word):
                            continue
...
def generate_dat_and_vocab_files(fnames, config):
    ...
    with codecs.open(config['datname'], 'w',  encoding='utf-8', errors='ignore') as datfile:
    ...
    with codecs.open(config['vocabname'], 'w',  encoding='utf-8', errors='ignore') as vocabfile:
